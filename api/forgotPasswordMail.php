...
if(isset($_POST['fp_email'])){
    ...
    $fp_code    = $_POST['fp_verification'];
    $fp_fname  =$_POST['fp_fname'];
    ...
    if(isset($postData)){
        $fp_email = $postData -> fp_email;
        $fp_code = $postData -> fp_code;
        $fp_fname = $postData -> fp_fname;


        // error_log(json_encode($fp_email),0);
        // error_log(json_encode($fp_code),0);
        // error_log(json_encode($fp_fname),0);
...
try {
    ...
    $mail->addAddress("$fp_email");               
    $mail->addReplyTo('plantipshop@gmail.com', 'PLANTIP');
    
    $mail->isHTML(true);                                  
    $mail->Subject = 'Plantip Forgot Password Code';
    $mail->Body    = "
    <style>
    table, td, div, h1, p {
          ...
          <table role='presentation' style='width:94%;max-width:600px;border:none;border-spacing:0;text-align:left;font-family:Arial,sans-serif;font-size:16px;line-height:22px;color:#fff;'>
            ...
            <tr>
              <td style='padding:30px;background-color:#ffffff; color:#404040;'>
                <h1 style='margin-top:0;margin-bottom:16px;font-size:26px;line-height:32px; color:#404040;font-weight:bold;letter-spacing:-0.02em;'>Hey $fp_fname!</h1>
                <p style='margin:0; color:#404040;'>Please enter the forgot password code to change your password.</p>
              </td>
            </tr>
            <tr>
              <td style='padding:35px 30px 11px 30px;font-size:0;background-color:#ffffff;border-bottom:1px solid #f0f0f5;border-color:rgba(201,201,207,.35);'>
                ...
                margin-right:auto;
                font-size: 20px'>
                  <p style='margin:0; text-align:center;'>$fp_code</p>
                </div>
                <!--[if mso]>
                </td>
                </tr>
