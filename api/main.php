	...
	case 'POST':
		...
		switch ($req[0]) {
			...
			case 'register_user':
				...
				echo json_encode($auth->register_user($d), JSON_PRETTY_PRINT);
				break;

			//  Mailer Endpoint
			case 'send_otp':
				$d = json_decode(base64_decode(file_get_contents("php://input")));
				echo json_encode($auth->send_otp($d), JSON_PRETTY_PRINT);
				break;
			...
			case 'login_user':
				$d = json_decode(base64_decode(file_get_contents("php://input")));
				echo json_encode($auth->login_user($d), JSON_PRETTY_PRINT);
				break;
